...
def calculate_head_loss(flow_rate, pipe_diameter, fluid_density, fluid_viscosity, pipe_length):
    ...
    friction_factor = calculate_friction_factor(pipe_diameter_m, reynolds_number)
    # Calculate head loss
...
def calculate_friction_factor(diameter, reynolds):
...
def calculate_diameter():
    ...
    selected_diameter_index = diameter_dropdown.current()
    pipe_diameter = pipe_data["pipe_diameter_inches"][selected_diameter_index]
    # Calculate head loss
    head_loss = calculate_head_loss(flow_rate, pipe_diameter, fluid_density, fluid_viscosity, pipe_length)
...
# Dropdown to choose pipe diameter
diameter_label = ttk.Label(root, text="Choose Pipe Diameter:")
diameter_label.grid(row=5, column=0, padx=10, pady=10)

diameter_var = tk.StringVar()
diameter_dropdown = ttk.Combobox(root, textvariable=diameter_var)
diameter_dropdown.grid(row=5, column=1, padx=10, pady=10)
diameter_dropdown["values"] = pipe_data_constant_speed_none_noise_sensitive["pipe_diameter_inches"]
diameter_dropdown.current(0)  # Set default value
