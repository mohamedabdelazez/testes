...
def calculate_diameter():
    ...
    for min_flow, max_flow in pipe_data["flow_range_gpm_at_" + selected_hours]:
        if min_flow <= flow_rate <= max_flow:
            pipe_diameter = pipe_data["pipe_diameter_inches"][pipe_data["flow_range_gpm_at_" + selected_hours].index((min_flow, max_flow))]
            inner_diameter = pipe_material_inner_diameter_inches[material_var.get()][pipe_data["pipe_diameter_inches"].index(pipe_diameter)]
            external_diameter = pipe_material_outer_diameter_inches[material_var.get()][pipe_data["pipe_diameter_inches"].index(pipe_diameter)]
            ...
            # Calculate head loss using inner diameter
            head_loss = calculate_head_loss(flow_rate, inner_diameter, fluid_density, fluid_viscosity, pipe_length, material_var.get())
            head_loss_label.config(text=f"Head Loss: {head_loss:.2f} meters")
            # Calculate velocity using inner diameter
