...
# GUI construction
root = tk.Tk()
root.title("Ashre Pipe Diameter and Head Loss Calculator")
...
def calculate_diameter():
    ...
    if manual_diameter_var.get():
        ...
        for min_flow, max_flow in pipe_data["flow_range_gpm_at_" + selected_hours]:
            if min_flow <= flow_rate <= max_flow:
                ...
                break
	# Define pipe material weights
    pipe_material_weights = {
        "Black Steel schd 40": 1.2664,  # kg/m
        "Black Steel schd 80": 1.619,  # kg/m
        "Copper": 1.044,  # kg/m
    # Add more materials and their weights here
    }
    ...
    pipe_weight = selected_diameter * pipe_length * pipe_material_weights[material]
    
    # Calculate water weight
    water_density = 998
    pi = 3.14159
    #pipe_inner_area
    water_weight = (math.pi * (inner_diameter/12)**2 / 4) * pipe_length * water_density
...
def export_to_excel():
    ...
    headers = ["Nominal Diameter (in)", "Internal Diameter (in)", "External Diameter (in)", "Head Loss (m)", "Velocity (ft/s)", "Pipe Weight (kg)", "pipe+water Weight (kg)"]
    ...
    for i, data in enumerate(calculation_data, start=2):
        ...
        sheet.cell(row=i, column=7, value=data["total_weight"]).alignment = Alignment(horizontal='center', vertical='center')

    # Adjust column widths for calculation sheet
    column_widths = [20, 20, 20, 15, 15, 15, 15 ,20, 20]  # Adjust according to your preference
